...
/**
 * Pruebas unitarias avanzadas del CarritoCompra.
 * Persona 4 — Mockito + pruebas avanzadas
 *
 * Cubre:
 *  - Operaciones básicas del carrito (sin mock)
 *  - Cálculo de total con impuesto y descuento (con mock)
 *  - Verificación de llamadas al ServicioPrecio
 *  - Pruebas parametrizadas de montos
 *  - Casos límite y excepcionales
 *  - Agrupación con @Nested
 */
...
class CarritoCompraTest {
    ...
    void tearDown() throws Exception {
        closeable.close();
        System.out.println("Prueba finalizada. Items en carrito: " + carrito.cantidadProductos());
    }

    // =========================================================
    // GRUPO 1 — Operaciones básicas (sin mock de cálculo)
    // =========================================================
    ...
    class OperacionesBasicas {
        ...
        void testAgregarDosProductos() {
            carrito.agregar(productoA, 1);
            carrito.agregar(productoB, 2);
            assertEquals(2, carrito.cantidadProductos());
        }
        ...
        void testAgregarProductoDuplicadoActualizaCantidad() {
            carrito.agregar(productoA, 1);
            carrito.agregar(productoA, 3);
            assertEquals(1, carrito.cantidadProductos(), "No debe haber duplicados, solo actualizar");
        ...
        void testResumenCarritoVacio() {
            assertEquals("Carrito vacío", carrito.obtenerResumen());
        }
    ...
    class CalculoTotalConMock {
        ...
        void testTotalSinImpuestoNiDescuento() {
            carrito.agregar(productoA, 1); // 2500.0
        ...
        void testTotalConImpuesto18() {
            carrito.agregar(productoA, 1); // subtotal = 2500.0
            when(servicioPrecioMock.calcularImpuesto(2500.0)).thenReturn(450.0); // 18%
            when(servicioPrecioMock.calcularDescuento(2500.0)).thenReturn(0.0);
        ...
        void testTotalConDescuento10() {
            carrito.agregar(productoA, 1); // subtotal = 2500.0
            when(servicioPrecioMock.calcularImpuesto(2500.0)).thenReturn(0.0);
            when(servicioPrecioMock.calcularDescuento(2500.0)).thenReturn(250.0); // 10%
        ...
        void testTotalMultiplesProductosConMock() {
            carrito.agregar(productoA, 1); // 2500
            carrito.agregar(productoB, 2); //  160
            double subtotal = 2660.0;

            when(servicioPrecioMock.calcularImpuesto(subtotal)).thenReturn(478.8);  // 18%
            when(servicioPrecioMock.calcularDescuento(subtotal)).thenReturn(266.0); // 10%
        ...
        void testResumenContieneTotal() {
            carrito.agregar(productoB, 1); // 80.0
            ...
            String resumen = carrito.obtenerResumen();
            assertTrue(resumen.contains("Mouse"), "El resumen debe contener el nombre del producto");
            assertTrue(resumen.contains("94.40") || resumen.contains("94,40"),
                       "El resumen debe contener el total correcto");
        }
    }
    ...
    class VerificacionInteracciones {
        ...
        void testLlamaAmbosMétodos() {
            carrito.agregar(productoB, 2); // subtotal = 160.0
            carrito.calcularTotal();
        ...
        @Test
        @DisplayName("No se llama al servicio si el carrito está vacío")
    ...
    class PruebasParametrizadas {
        ...
        void testAgregarCantidadesValidas(int cantidad) {
            carrito.agregar(productoB, cantidad);
        ...
        void testSubtotalSegunCantidad(int cantidad, double subtotalEsperado) {
            ...
            assertEquals(subtotalEsperado,
                         carrito.getItems().get(0).getSubtotal(), 0.001);
        }
        ...
        void testTotalConDiferentesImpuestos(double impuesto, double totalEsperado) {
            carrito.agregar(productoA, 1); // subtotal = 2500
    ...
    class ValidacionesYExcepciones {
        ...
        void testNoAgregarProductoNoDisponible() {
            ...
            assertThrows(IllegalStateException.class,
                () -> carrito.agregar(noDisponible, 1),
                "Debe lanzar excepción al agregar producto no disponible");
        }
        ...
        void testRemoverProductoInexistente() {
            carrito.agregar(productoA, 1);
            assertThrows(IllegalArgumentException.class,
                () -> carrito.remover("ID_INEXISTENTE"),
                "Debe lanzar excepción al remover producto que no existe");
        }
    ...
    class CasosLimite {
        ...
        void testCarritoCienProductos() {
            ...
            assertEquals(100, carrito.cantidadProductos());
        }
        ...
        void testDescuentoMayorQueSubtotal() {
            carrito.agregar(productoB, 1); // subtotal = 80.0
            when(servicioPrecioMock.calcularImpuesto(80.0)).thenReturn(0.0);
            when(servicioPrecioMock.calcularDescuento(80.0)).thenReturn(100.0);
            assertEquals(-20.0, carrito.calcularTotal(), 0.001,
                         "Un descuento mayor al subtotal puede resultar en total negativo");
        }
    }
}
